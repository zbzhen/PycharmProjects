...
axes.plot([-1,1], [1,-1], color='blue', linestyle='solid', marker='',
         markerfacecolor='blue', linewidth=8)
for i in range(4):
    x,y = xedge[i], yedge[i]
...
theta = -0.234
axes.annotate('',xy=(1,1),xytext=(theta,-theta),arrowprops=dict(arrowstyle="<-",connectionstyle="arc3"),
              fontsize = 'xx-large',
              style =  'oblique',
              fontvariant =  'small-caps')

axes.plot([theta], [-theta], color='red', linestyle='dashed', marker='o',
            markerfacecolor='red', markersize=12)

ftz = 40; a = ftz*0.001
axes.text(theta, -theta,"$(\\theta,1-\\theta)$",fontsize = ftz, verticalalignment='top', horizontalalignment='right')
...
plt.savefig("RnoLGLpoints.pdf",dip=120)
plt.show()
help(plt.annotate)
