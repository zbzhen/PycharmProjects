...
class EleDG2r:
    ...
    def NDe(self, j, i): #第一个变量j为主单元的边序号，i为邻单元边序号
        ...
        return 0.5*self.ss*L[j]*Intergral_2().guass_leg_1(f, (-1, 1))
        #这个0.5是因为边界积分（-1,1）所产生

    #一个求导
    def De(self, j, i): #第一个变量j为主单元的边序号
        L = [self.L0, self.L1, self.L2, self.L3]
        nk = [self.nk0, self.nk1, self.nk2, self.nk3]
        def f(s):
            Dphi_j = Dphi()[j](s)
            phi_i = np.array([phi()[i](s)])
            ...
            return ans
        return 0.25*Intergral_2().guass_leg_1(f, (-1, 1))*L[j]
...
def KF(f, D, (H, L), ee, ss):
    ...
    bbbb = np.array([aa.NDe(0, 2)*w+aa.De(0, 2)-aa.De(2, 0).T*ee, aa.NDe(1, 3)*w+aa.De(1, 3)-aa.De(3, 1).T*ee,
                     aa.NDe(2, 0)*w+aa.De(2, 0)-aa.De(0, 2).T*ee, aa.NDe(3, 1)*w+aa.De(3, 1)-aa.De(1, 3).T*ee])*(-1)
    #组装中间项与右端项
    for i in range(H*L):
