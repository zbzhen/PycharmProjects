...
class Ele2r:
    ...
    def fe(self):
        a, b = self.f(self.D[0][0], self.D[0][1]), self.f(self.D[1][0], self.D[1][1])
        c, d = self.f(self.D[2][0], self.D[2][1]), self.f(self.D[3][0], self.D[3][1])
        ff = np.array([4*a + 2*b + c + 2*d, 2*a + 4*b + 2*c + d, a + 2*b + 4*c + 2*d, 2*a + b + 2*c + 4*d])
        return self.b*self.a*ff/36

"""
D = [[0, 0], [0.25, 0], [0.25, 0.25], [0, 0.25]]
def f(x, y):
...
if __name__ == '__main__' :
    ...
    print np.log(erro(32, 32)/erro(64, 64))/np.log(2)
    """
