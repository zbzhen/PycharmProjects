...
class Ele2t:
    ...
    def fe(self):
        a, b, c = self.f(self.D[0][0], self.D[0][1]), self.f(self.D[1][0], self.D[1][1]), self.f(self.D[2][0], self.D[2][1])
        ff = np.array([2*a + b + c, a + 2*b + c, a + b + 2*c])
        return ff*abs(tradet(self.D))/24
